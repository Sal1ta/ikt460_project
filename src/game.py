...
class GameManager:
    ...
    def assign_players_colors(self, number_of_players, verbose=False):
        # Two and four players must use opposite pairs Three, five, and six
        # players use the natural colour sets directly
        if number_of_players == 2:
            ...
            all_colours = BASE_COLORS.copy() + OPPOSITE_COLORS.copy()
            omitted_colour = str(np.random.choice(all_colours))
            self.player_colors = [colour for colour in all_colours if colour != omitted_colour]
        elif number_of_players == 6:
            self.player_colors = BASE_COLORS.copy() + OPPOSITE_COLORS.copy()
        else:
            raise ValueError("Supported player counts are 2, 3, 4, 5, and 6.")
